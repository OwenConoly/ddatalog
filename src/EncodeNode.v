(* Here we define the higher level operational semantics of how a trie join works. This is abstracted only
   ...
   The compiler ([EncodeLayout]) turns a lowered datalog program (relations and
   ...
   [hw_node_correct] reduces this to a per-rule bridge [hw_rule_matches].  EncodeNode is
   compiler-agnostic: it never mentions the compiler's [lowered_rule] AST.  Proving a *compiled*
   hardware program implements its source program is the job of [EncodeLayoutCorrect]; the
   distributed version is [EncodeDistributed]. *)
...
(* The reference programs this node is verified against are ordinary [Datalog] programs over
   the numeric ids the hardware uses.  EncodeNode never mentions the compiler's [lowered_rule]
   ...
   compiler's job in [EncodeLayoutCorrect].) *)
...
(* Per-rule bridge: hardware rule [hr] (with trie table [tries]) is *correct* for datalog rule
   ...
   choice of [env] is immaterial.)  Everything else reduces to this; [EncodeLayoutCorrect]
